...
function finalize() {
  ...
  arr.forEach((element) => {
    ...
    if (dateInput.value !== "") {
      const goalValue = element.querySelector("#goal-value").value;
      const goalStatus = element.querySelector("#goal-status").value;
      ...
      dailies.forEach((element) => {
        ...
        if (value !== "") {
          dailyArr.push({ date: string, value });
        }
      });
    }
  });
  ...
  fetch(`http://localhost:${serverPort}/debug/dailyactivity`, {
    ...
    if (res.status === 400) {
      document.querySelector(
        "#status-space"
      ).innerHTML = `<p>UserID must be set</p>`;
